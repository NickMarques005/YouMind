...
const DataDetailsModal: React.FC<DataDetailsModalProps> = ({ loading, setLoading, closeModal, userData, HandleResponseAppError, HandleResponseAppSuccess }) => {
    ...
    const { showGenderOptions, handleGenderOptions } = UseHandleProfileDetails();
        ...
        <View style={styles.container}>
            {
                showGenderOptions ?
                    ...
                    <>
                        ...
                        <TouchableOpacity disabled={!isButtonEnabled || loading} style={[styles.button, { opacity: loading ? 0.5 : isButtonEnabled ? 1 : 0.5 }]} onPress={() => handleUpdateProfileData(userDataToUpdate, closeModal)}>
                            {
                                loading ?
