...
function Login({ }) {
    ...
    const loginStyles = LoginStyles(userType);
    ...
    const LoginService = async () => {
        console.log("(Login) ENTER LOGIN PATIENT: ", loginData);
        /*if (loginData.email && loginData.password) {
            ...
            try {
                ...
                if (response?.tokens) {
                    ...
                    console.log("(Login) Houve algum erro ao fazer SignIn...");
                }
                else {
                ...
                console.log("(Login) Houve um erro no login!!!");
            }
        }
        else {
    ...
    const handleRememberMe = () => {
        console.log("(Login) SAVE TOKEN!! * Remember me");
        setRememberMe(!rememberMe);
    }

    const backgroundHeader = userType === 'doctor' ? images.generic_images.auth.doctor.doctor_background_auth : images.generic_images.auth.patient.patient_background_auth.jpg;
    const loginButton = userType === 'doctor' ? images.generic_images.auth.doctor.doctor_button_auth : images.generic_images.auth.patient.patient_button_auth.jpg;
                    ...
                    <View style={loginStyles.form}>
                        ...
                        <View style={loginStyles.inputsMainView}>
                            <View style={[loginStyles.viewInput, { borderColor: `${userType === 'doctor' ? '#4193b0' : '#a541b0'}` }]}>
                                <Image style={{ width: 25, height: 25 }} source={login_icon.email} />
                                <TextInput
                            ...
                            <View style={loginStyles.userPassOptionsContainer}>
                                <View style={[loginStyles.viewInput, { borderColor: `${userType === 'doctor' ? '#4193b0' : '#a541b0'}` }]}>
                                    <Image style={{ width: 25, height: 25 }} source={login_icon.password} />
                                    <TextInput
...
const LoginStyles = (userType: "doctor" | "patient" | "") => {
